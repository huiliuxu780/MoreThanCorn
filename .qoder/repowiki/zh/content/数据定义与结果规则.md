...
## 产品概述
本专题聚焦“数据定义”和“结果规则（Result RuleSet）”两类配置能力：前者为 Data Asset 提供字段语义层，后者将 AI 结构化输出解释为评分、风险、等级与派生标签。二者共同构成“输入可理解、输出可度量”的质量评价基座，支撑分析任务与质检结果的稳定运行与可追溯。

- 目标用户：质量运营、规则配置人员、数据分析与质检实施人员。
- 核心价值：
  - 数据定义：把原始数据映射为可被分析与筛选的语义字段，支持推断、必填约束与发布版本。
  - 结果规则：以版本化规则集对 Effective Result 求值，产出 Score/Risk/Level/Derived Labels，并支持重算与审计。
- 适用场景：坐席质检、工单质检、AI Agent 质检等基于结构化输出的质量评估与业务解释。
