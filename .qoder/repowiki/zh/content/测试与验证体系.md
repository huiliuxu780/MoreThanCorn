...
## 更新摘要
**变更内容**
- 新增 UI 标准化检查脚本 `scripts/check-ui-standard.mjs`，实现组件标准静态检查，禁止原生 select/option/裸 input/textarea/window.alert-confirm-prompt/手搓 switch
- 新增视觉回归测试脚本 `scripts/check-visual-regression.mjs`，提供视觉基线回归 + 设计令牌 DOM 断言功能
- 新增 `scripts/ui-allowlist.json` 配置文件，管理 UI 标准的例外情况，支持 allowlist 机制
- 增强测试验证体系的自动化能力，涵盖代码规范、视觉一致性、组件标准等多个维度
