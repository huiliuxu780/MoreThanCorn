...
## 更新摘要
**变更内容**
- 新增 P7 验收测试套件 `server/tests/test_p7_acceptance.py`，覆盖 workflow-fixed 节点版本钉定、workflow-exec 动态模式执行、workflow-select 路由选择三大场景的端到端测试
- 增强工作流执行器支持：workflow-fixed 支持 inputMapping 输入映射和 pinnedVersionId 版本钉定；workflow-exec 支持 dynamic 模式从输入绑定获取工作流代码；workflow-select 支持语义路由和 miss 分支处理
- 完善工作流迁移机制：旧版 agent 三键自动迁移为 workflow 三连（workflow-fixed/workflow-select/workflow-exec）
- 强化验收测试基础设施：新增完整的后端 pytest 分层测试，确保工作流核心功能的稳定性和正确性
...
- **UI 标准化检查（新增）**：通过静态代码分析确保组件使用规范，禁止原生 HTML 元素和不符合设计系统的组件。
- **视觉回归测试（新增）**：通过像素级比对确保 UI 视觉一致性，防止样式变更导致的视觉回归。
- **P7 工作流验收测试（新增）**：新增 comprehensive acceptance test suite 覆盖 workflow-fixed 节点版本钉定、workflow-exec 动态模式执行、workflow-select 路由选择的端到端测试场景。
...
US["UI 标准化检查<br/>check-ui-standard.mjs"]
VR["视觉回归测试<br/>check-visual-regression.mjs"]
end
subgraph "验收测试"
...
TP7["P7 工作流验收测试<br/>test_p7_acceptance.py"]
end
FE --> |HTTP| A
E2E --> |CDP| FE
VT --> |纯函数测试| FE
...
- [scripts/check-ui-standard.mjs:1-74](file://scripts/check-ui-standard.mjs#L1-L74)
- [scripts/check-visual-regression.mjs:1-121](file://scripts/check-visual-regression.mjs#L1-L121)
- [server/tests/test_phase_e.py:1-202](file://server/tests/test_phase_e.py#L1-L202)
- [server/tests/test_p7_acceptance.py:1-101](file://server/tests/test_p7_acceptance.py#L1-L101)
...
- 后端 pytest 分层
  ...
  - **P7 工作流验收测试（新增）**：workflow-fixed 节点版本钉定与输入映射、workflow-exec 动态模式执行、workflow-select 语义路由与 miss 分支处理。
...
- **UI 标准化检查（新增）**
  - **组件标准静态检查**：禁止使用原生 select/option/裸 input/textarea/window.alert-confirm-prompt/手搓 switch
  - **Allowlist 管理机制**：通过 ui-allowlist.json 管理例外情况，只减不增原则
  - **Components/WF 台账比对**：确保 components/wf 目录下的文件都在登记清单中
  - **Shadcn/UI 豁免**：components/ui/ 目录下的组件不受限制
  - **种子模式**：支持 --seed 参数生成初始 allowlist 基线
- **视觉回归测试（新增）**
  - **视觉基线比对**：通过像素级差异检测确保 UI 视觉一致性
  - **设计令牌断言**：验证节点卡片宽度、圆角、画布背景色、抽屉宽度等设计令牌
  - **多场景截图**：覆盖画布视图、各节点抽屉等不同界面状态
  - **差异报告**：生成差异热图和详细报告，便于定位视觉回归问题
  - **基线管理**：支持 --update-baseline 参数更新基准图片
- **前端 Vitest 单元测试**
...
- **P7 工作流验收测试（新增）**
  - **workflow-fixed 节点测试**：验证固定工作流节点的版本钉定（pinnedVersionId）和输入映射（inputMapping）功能，确保子工作流使用指定版本而非草稿。
  - **workflow-exec 动态模式测试**：验证动态模式下从输入绑定获取工作流代码的能力，支持运行时动态选择执行的工作流。
  - **workflow-select 路由测试**：验证语义路由功能和 miss 分支处理，当没有匹配的工作流时能够正确走 miss 分支。
  - **端到端测试场景**：创建子工作流 → 发布版本 → 创建父工作流 → 配置节点 → 执行验证 → 断言输出结果。
- 前端 E2E 脚本
...
- [scripts/check-ui-standard.mjs:1-74](file://scripts/check-ui-standard.mjs#L1-L74)
- [scripts/check-visual-regression.mjs:1-121](file://scripts/check-visual-regression.mjs#L1-L121)
- [scripts/ui-allowlist.json:1-3](file://scripts/ui-allowlist.json#L1-L3)
...
- 版本与草稿
  ...
  - **P7 工作流增强（新增）**：workflow-fixed 节点支持 pinnedVersionId 版本钉定，确保子工作流使用指定版本而非草稿；inputMapping 支持从上游节点输出映射到子工作流输入。
- 资源依赖与引用
...
- **UI 标准化检查增强（新增）**
  - **组件标准强制执行**：禁止使用原生 HTML 元素和不规范组件，确保设计系统一致性
  - **Allowlist 基线管理**：通过 ui-allowlist.json 管理例外情况，支持种子模式生成初始基线
  - **Components/WF 台账管控**：确保 components/wf 目录下只有登记的文件存在
  - **静态分析集成**：可在 CI 流程中集成，阻止不符合规范的代码提交
- **视觉回归测试增强（新增）**
  - **像素级差异检测**：使用 pixelmatch 进行精确的视觉回归检测
  - **设计令牌验证**：验证关键设计令牌如节点宽度(300px)、圆角(8px)、画布背景(#EEF1F6)、抽屉宽度(360px)
  - **多场景覆盖**：覆盖画布视图、各节点抽屉等不同界面状态的视觉回归检测
  - **差异可视化**：生成差异热图，直观展示视觉回归的具体位置
- **P7 工作流执行增强（新增）**
  - **workflow-fixed 节点**：支持固定工作流执行，包含版本钉定（versionPolicy=pinned + pinnedVersionId）和输入映射（inputMapping）功能，确保子工作流使用指定版本并正确映射输入参数。
  - **workflow-exec 动态模式**：支持动态模式（mode=dynamic），从输入绑定获取工作流代码，实现运行时动态选择执行的工作流。
  - **workflow-select 语义路由**：支持候选工作流的语义路由选择，未命中时走 miss 分支，提供 selected/workflowCode/workflowName/workflowDesc 输出。
  - **工作流迁移机制**：旧版 agent 三键（agent/agent-select/agent-exec）自动迁移为 workflow 三连（workflow-fixed/workflow-select/workflow-exec），保持向后兼容。

```mermaid
sequenceDiagram
participant US as "UI标准化检查"
participant VR as "视觉回归测试"
participant TP7 as "P7工作流验收测试"
participant API as "后端 API"
participant FE as "前端应用"
participant CHROME as "Chrome 浏览器"
Note over US,VR : 新增测试验证能力
US->>US : 扫描 src 目录下的 TSX/JSX 文件
US->>US : 检查违规模式 (select/input/textarea等)
US->>US : 比对 ui-allowlist.json 基线
US-->>US : 输出检查结果
VR->>API : 创建临时工作流
API-->>VR : 返回 workflow.id
VR->>CHROME : 启动无头 Chrome
CHROME->>FE : 访问工作流编辑器
FE-->>CHROME : 渲染画布界面
VR->>CHROME : 添加节点并截图
CHROME->>FE : 执行节点操作
...
VR-->>VR : 生成差异报告和热图
TP7->>API : 创建子工作流并发布版本
API-->>TP7 : 返回 workflow.id 和 versionId
TP7->>API : 创建父工作流并配置 workflow-fixed 节点
API-->>TP7 : 返回父工作流配置
TP7->>API : 执行工作流并验证输出
API-->>TP7 : 返回执行结果和输出
TP7->>TP7 : 断言版本钉定和输入映射正确性
```

**图表来源**
- [scripts/check-ui-standard.mjs:22-74](file://scripts/check-ui-standard.mjs#L22-L74)
- [scripts/check-visual-regression.mjs:28-121](file://scripts/check-visual-regression.mjs#L28-L121)
- [scripts/ui-allowlist.json:1-3](file://scripts/ui-allowlist.json#L1-L3)
- [server/tests/test_p7_acceptance.py:43-68](file://server/tests/test_p7_acceptance.py#L43-L68)
...
- [scripts/check-ui-standard.mjs:13-20](file://scripts/check-ui-standard.mjs#L13-L20)
- [scripts/check-visual-regression.mjs:68-93](file://scripts/check-visual-regression.mjs#L68-L93)
- [server/tests/test_p7_acceptance.py:43-101](file://server/tests/test_p7_acceptance.py#L43-L101)
...
- 依赖与集成边界
  ...
  - **UI 标准化检查增强（新增）**：check-ui-standard.mjs 依赖 Node.js 文件系统 API，可在任何环境中执行，无需浏览器依赖。
  - **视觉回归测试增强（新增）**：check-visual-regression.mjs 依赖 puppeteer-core、pngjs、pixelmatch，需要 Chrome 浏览器和前后端服务同时运行。
  - **P7 工作流测试增强（新增）**：test_p7_acceptance.py 依赖 FastAPI TestClient 和数据库会话，需要后端服务和数据库可用。
- 业务约束
  ...
  - **UI 标准化增强（新增）**：禁止使用原生 select/option/input/textarea 元素，必须使用 shadcn/ui 组件；禁止 window.alert/confirm/prompt；禁止手写 switch 组件；components/wf 目录下的文件必须在 allowlist 中登记。
  - **视觉回归增强（新增）**：节点卡片宽度固定为 300px，圆角为 8px；画布背景色为 #EEF1F6；抽屉宽度为 360px；视觉差异率超过 0.5% 视为回归失败。
  - **P7 工作流增强（新增）**：workflow-fixed 节点必须配置有效的 workflowId 和 versionPolicy；workflow-exec 动态模式需要从输入绑定获取 workflowCode；workflow-select 必须配置至少一个候选工作流；版本钉定时必须提供有效的 pinnedVersionId。
...
- [scripts/check-ui-standard.mjs:13-20](file://scripts/check-ui-standard.mjs#L13-L20)
- [scripts/check-visual-regression.mjs:15-22](file://scripts/check-visual-regression.mjs#L15-L22)
- [server/tests/test_p7_acceptance.py:43-101](file://server/tests/test_p7_acceptance.py#L43-L101)
...
- 本地开发
  ...
  - **UI 标准化检查（新增）**：运行 node scripts/check-ui-standard.mjs 进行组件标准检查，使用 --seed 参数生成初始 allowlist 基线。
  - **视觉回归测试（新增）**：运行 node scripts/check-visual-regression.mjs 进行视觉回归检测，使用 --update-baseline 参数更新基准图片。
  - **P7 工作流验收测试（新增）**：运行 pytest server/tests/test_p7_acceptance.py -v 验证 workflow-fixed、workflow-exec、workflow-select 节点的功能。
- 持续集成
  ...
  - **UI 标准化检查增强（新增）**：check-ui-standard.mjs 作为代码质量门禁，确保组件使用符合设计规范，可在 CI 中阻止不符合规范的代码提交。
  - **视觉回归测试增强（新增）**：check-visual-regression.mjs 作为发布质量门禁，确保 UI 视觉一致性，防止样式变更导致的视觉回归。
  - **P7 工作流测试增强（新增）**：test_p7_acceptance.py 作为工作流核心功能的验收测试，确保 workflow-fixed、workflow-exec、workflow-select 节点的正确性和稳定性。
...
### P7 工作流验收测试（新增）
- **workflow-fixed 节点版本钉定测试（A4）**：
  - **子工作流创建与发布**：创建子工作流并发布 v1 版本，获取 versionId。
  - **草稿修改隔离**：修改子工作流草稿（end 节点输出改为固定值 CHANGED），确保不影响已发布版本。
  - **父工作流配置**：创建父工作流，配置 workflow-fixed 节点，设置 versionPolicy=pinned 和 pinnedVersionId=v1。
  - **输入映射验证**：配置 inputMapping 将父工作流的 userQuery 映射到子工作流的 userQuery。
  - **版本钉定断言**：执行父工作流，验证子工作流使用 v1 版本而非草稿，输出为 hello-mapping 而非 CHANGED。
- **workflow-exec 动态模式测试（A5）**：
  - **动态模式配置**：创建 workflow-exec 节点，设置 mode=dynamic，从输入绑定获取 workflowCode。
  - **运行时工作流选择**：通过输入参数动态指定要执行的工作流代码。
  - **执行结果验证**：验证动态模式能够正确执行指定的工作流并返回预期输出。
- **workflow-select 路由测试（A6）**：
  - **候选工作流配置**：配置 workflow-select 节点，设置候选工作流列表。
  - **语义路由验证**：验证根据输入查询语义选择最合适的工作流。
  - **miss 分支处理**：当没有匹配的工作流时，验证能够正确走 miss 分支。
  - **路由输出断言**：验证 selected、workflowCode、workflowName、workflowDesc 等输出字段。
- **端到端测试场景**：
  - **完整工作流执行**：从创建工作流到执行验证的完整测试流程。
  - **数据库会话管理**：使用独立的数据库会话确保测试隔离性。
  - **错误处理验证**：验证各种错误场景下的正确处理。
...
### UI 标准化检查（新增）
- **组件标准静态检查**：
  - **禁止原生 HTML 元素**：检测并阻止使用原生 select、option、input、textarea 元素
  - **禁止原生对话框**：检测 window.alert、window.confirm、window.prompt 调用
  - **禁止手写组件**：检测 role="switch" 等手写组件实现
  - **Shadcn/UI 豁免**：components/ui/ 目录下的组件不受限制
- **Allowlist 管理机制**：
  - **基线管理**：通过 ui-allowlist.json 管理例外情况，支持只减不增原则
  - **种子模式**：支持 --seed 参数生成初始 allowlist 基线
  - **增量检查**：检测新增的违规项，确保代码质量持续提升
  - **台账管控**：确保 components/wf 目录下只有登记的文件存在
- **CI 集成支持**：
  - **快速执行**：无需浏览器依赖，可在任何环境中快速执行
  - **明确报错**：详细的违规位置和数量，便于开发者快速修复
  - **基线更新**：支持渐进式改进，逐步减少违规项

### 视觉回归测试（新增）
- **视觉基线比对**：
  - **像素级差异检测**：使用 pixelmatch 进行精确的视觉回归检测
  - **多场景覆盖**：覆盖画布视图、各节点抽屉等不同界面状态
  - **差异阈值**：设置 0.5% 的差异率阈值，平衡敏感度和误报率
  - **差异可视化**：生成差异热图，直观展示视觉回归的具体位置
- **设计令牌断言**：
  - **节点卡片**：验证宽度(300px)、圆角(8px)等设计令牌
  - **画布背景**：验证背景色(#EEF1F6)等全局样式
  - **抽屉尺寸**：验证抽屉宽度(360px)等布局令牌
  - **响应式适配**：确保在不同屏幕尺寸下的视觉一致性
- **基线管理**：
  - **基线更新**：支持 --update-baseline 参数更新基准图片
  - **版本控制**：基线图片纳入版本控制，确保回归检测的准确性
  - **环境隔离**：支持不同环境的独立基线管理
...
### Phase C（事件通道与新节点执行器）
- **事件通道**：双通道架构（CONTROL/CONTENT）、Trace/Sspan 追踪、持续时间统计
