...
- **新增 RuleSet 范围限制测试**：验证 follow_latest 策略必须显式声明 RuleSet 作用域，防止串用其他规则集的最新版本
...
- **RuleSet 范围限制**：follow_latest 策略必须显式声明 resultRuleSetId，防止串用其他规则集

**更新** 新增了 RuleSet 范围限制的完整测试覆盖，确保 follow_latest 策略只能在声明的规则集范围内解析最新版本，避免意外使用其他规则集的规则。
...
- **RuleSet 范围数据流**：任务创建 → RuleSet 验证 → 规则版本解析 → 范围限定 → 执行隔离

**更新** 新增了 RuleSet 范围数据流，确保 follow_latest 策略在任务创建时验证 RuleSet 作用域，并在执行时限定在指定规则集内解析最新版本。
...
- **RuleSet 范围约束**：follow_latest 策略必须显式声明 resultRuleSetId，禁止全库取最新版本，防止串用其他规则集

**更新** 新增了 RuleSet 范围约束，确保 follow_latest 策略只能在声明的规则集范围内解析最新版本，避免意外的规则混用。
...
- **RuleSet 范围专项**：pytest server/tests/test_p0_taskrun.py::test_follow_latest_rule_policy server/tests/test_p0_taskrun.py::test_follow_latest_requires_rule_set_scope -v

**更新** 新增了 RuleSet 范围专项测试，专门验证 follow_latest 策略的范围限制功能。

**章节来源**
- [server/tests/test_p0_taskrun.py:137-173](file://server/tests/test_p0_taskrun.py#L137-L173)
- [server/app/task_runner.py:53-78](file://server/app/task_runner.py#L53-L78)
- [server/app/routers/business.py:540-551](file://server/app/routers/business.py#L540-L551)
- [server/app/models.py:596-600](file://server/app/models.py#L596-L600)
- [server/alembic/versions/g039p2fix00001_rule_scope.py:1-32](file://server/alembic/versions/g039p2fix00001_rule_scope.py#L1-L32)
- [server/tests/_quality_setup.py:99-104](file://server/tests/_quality_setup.py#L99-L104)