...
## 更新摘要
**变更内容**
- 新增 Phase D-1（SDD 04 §0）评测能力：工作流级评测、Agent 级评测、人类评分验证、多评判模式覆盖
- 实现 EvalSample 模型支持 Agent/Workflow 双维度挂载，judge_result 字段记录最近一次评判结果
- 新增工作流级评测 API：eval-run、human-score、eval-summary、version-metrics
- 前端集成效果评测面板：样本管理、规则/模型/人评三种 Judge 模式、实时统计
- 新增 Phase D-1 验收测试：评测闭环、Prompt 生成、成员冻结摘要、指标统计
- 数据库迁移：eval_sample 支持 agent_id、workflow_id 可空、judge_result JSONB 字段、evolution_patch 表
...
## 核心业务流程
- 后端测试分层以"阶段/能力域"划分：Phase A（版本/路由/条件/挂载留痕）、Phase B（Agent 聚合根与发布闭环）、Phase C（事件通道/新节点执行器/记忆持久化）、Phase D-1（评测闭环/观测指标/Prompt 生成/成员冻结）、Runner（执行器/事件/SSE/递归保护）、P2（调度/重试/连接/指标）、P5（知识检索/MCP/发布引用快照）、业务深化（规则/批量/周期）、契约（校验七规则/注册表/黄金 fixture）、Agent 运行层（三型 Agent/挂载健康/发布同步）。
- **Phase D-1 评测增强（新增）**：工作流级评测支持样本逐个真实运行，规则 Judge（期望包含匹配）、模型 Judge（LLM 打 1-5 分）、人评覆盖（0-5 分），支持 Agent 级和 Workflow 级评测样本管理。
...
- 后端 pytest 分层
  ...
  - **Phase D-1（新增）**：评测样本管理（Agent/Workflow 双维度）、真实运行评测、规则/模型/人评三种 Judge 模式、评测统计汇总、版本指标分析、Prompt 自动生成、专家组成员冻结摘要。
...
- **前端效果评测面板（新增）**
  - **样本管理**：添加/删除评测样本，支持输入 JSON 和期望文本配置。
  - **多模式 Judge**：规则 Judge（期望包含匹配）、模型 Judge（LLM 打 1-5 分）、不 Judge（只看运行成败）。
  - **人评覆盖**：对评测结果进行人工打分（0-5 分），覆盖或补充机器 Judge 结果。
  - **实时统计**：显示总数、成功数、失败数、成功率、平均耗时等关键指标。
- 前端 E2E 脚本
...
- **Phase D-1 评测增强（新增）**
  - **EvalSample 模型**：支持 workflow_id 和 agent_id 双维度挂载，name/input/expected/judge_result 字段存储评测样本信息。
  - **Judge 结果**：judge_result 字段记录最近一次评判结果，包含 kind（rule/model/human）、score（数值分数）、passed（布尔判断）、note（备注）。
  - **评测统计**：eval-summary 接口提供 total/succeeded/failed/successRate/avgDurationMs 等关键指标。
  - **版本指标**：version-metrics 接口提供各版本的运行次数、成功率、失败案例等分析数据。
...
FE->>API : POST /api/workflows/{wid}/eval-samples (添加样本)
API->>DB : 创建 EvalSample(workflow_id, input, expected)
FE->>API : POST /api/workflows/{wid}/eval-run (judge=rule/model/human)
API->>RUN : create_run(execute_run)
RUN->>RUN : execute_run(加载版本快照)
RUN->>DB : 写事件(channel/trace/span)
RUN-->>API : workflow_completed/failed
API->>EVAL : 执行 Judge(rule/model)
EVAL->>DB : 更新 EvalSample.judge_result
FE->>API : GET /api/workflows/{wid}/eval-summary
API-->>FE : 评测统计(total/succeeded/failed/successRate)
FE->>API : POST /api/eval-samples/{sid}/human-score (人评覆盖)
API->>DB : 更新 judge_result(kind="human")
```

**图表来源**
- [server/app/routers/admin.py:618-703](file://server/app/routers/admin.py#L618-L703)
- [server/app/models.py:371-384](file://server/app/models.py#L371-L384)
- [server/alembic/versions/d029phased3001_judge_evolution.py:22-35](file://server/alembic/versions/d029phased3001_judge_evolution.py#L22-L35)
- [src/pages/wf-designer.tsx:2124-2203](file://src/pages/wf-designer.tsx#L2124-L2203)
- [src/services/wf-api.ts:381-397](file://src/services/wf-api.ts#L381-L397)
...
- 业务约束
  ...
  - **Phase D-1 评测增强（新增）**：
    - 评测样本必须包含有效输入（input 不为空）；期望输出（expected.text）可选。
    - 规则 Judge 要求期望文本存在于实际输出中；模型 Judge 依赖 LLM 服务可用性。
    - 人评分数范围限制在 0-5 之间，支持覆盖或补充机器 Judge 结果。
    - 评测运行触发 trigger="eval"，与正常业务运行区分。
    - 前端效果评测面板支持 rule/model/human 三种 Judge 模式切换。
- 前端测试增强：列表过滤和排序逻辑必须保持 URL 编码的一致性和稳定性。
...
- [server/tests/test_phase_c.py:120-142](file://server/tests/test_phase_c.py#L120-142)
- [server/app/routers/admin.py:658-670](file://server/app/routers/admin.py#L658-L670)
- [src/pages/wf-designer.tsx:2180-2203](file://src/pages/wf-designer.tsx#L2180-L2203)
...
- 本地开发
  ...
  - Phase B/C/D-1 专项测试：单独运行 test_phase_b.py、test_phase_c.py、test_phase_d1.py 验证新功能。
- 持续集成
  ...
  - Phase B/C/D-1 测试用例作为回归测试的一部分，确保版本管理和事件通道功能的稳定性。
  - 前端测试增强：Vitest 单元测试作为代码质量门禁，确保核心逻辑的正确性。
  - **Phase D-1 评测测试增强（新增）**：新增 test_phase_d1.py 作为评测能力的验收测试，确保工作流级评测、Agent 级评测、人类评分验证等功能正常工作。
...
### Phase D-1（评测闭环与观测指标）
- **评测样本管理**：Agent/Workflow 双维度样本创建、查询、删除，支持输入 JSON 和期望文本配置。
- **真实运行评测**：样本逐个真实运行（同步等待终态），支持指定 sampleIds 选择性评测。
- **多模式 Judge**：规则 Judge（期望包含匹配）、模型 Judge（LLM 打 1-5 分）、不 Judge（只看运行成败）。
- **人类评分验证**：0-5 分范围的人评打分，覆盖或补充机器 Judge 结果，支持备注说明。
- **评测统计汇总**：total/succeeded/failed/successRate/avgDurationMs 等关键指标统计。
- **版本指标分析**：各版本运行次数、成功率、失败案例等分析数据。
- **Prompt 生成**：根据名称和提示自动生成角色 Prompt。
- **成员冻结摘要**：专家组成员版本冻结，随版本返回 frozenMembers 信息。
...
### 前端效果评测面板（新增）
- **样本管理界面**：添加样本（名称、输入 JSON、期望文本）、删除样本、样本列表展示
- **多模式 Judge 选择**：规则 Judge（期望包含匹配）、模型 Judge（LLM 打 1-5 分）、不 Judge（只看运行成败）
- **人评覆盖功能**：对评测结果进行人工打分（👍 5 分、👎 1 分），实时更新 judge 结果
- **实时统计面板**：显示总数、成功数、失败数、成功率、平均耗时等关键指标
- **评测结果展示**：样本名称、运行状态、耗时、输出预览、Judge 结果、错误信息

**章节来源**
- [server/tests/test_phase_d1.py:1-101](file://server/tests/test_phase_d1.py#L1-L101)
- [server/app/routers/admin.py:618-703](file://server/app/routers/admin.py#L618-L703)
- [server/app/models.py:371-384](file://server/app/models.py#L371-L384)
- [server/alembic/versions/d028phased1001_eval_sample_agent.py:21-31](file://server/alembic/versions/d028phased1001_eval_sample_agent.py#L21-L31)
- [server/alembic/versions/d029phased1002_eval_sample_workflow_nullable.py:21-29](file://server/alembic/versions/d029phased1002_eval_sample_workflow_nullable.py#L21-L29)
- [server/alembic/versions/d029phased3001_judge_evolution.py:22-42](file://server/alembic/versions/d029phased3001_judge_evolution.py#L22-L42)
- [src/pages/wf-designer.tsx:2124-2203](file://src/pages/wf-designer.tsx#L2124-L2203)
- [src/services/wf-api.ts:381-397](file://src/services/wf-api.ts#L381-L397)
- [src/components/agent-ops-panels.tsx:156-172](file://src/components/agent-ops-panels.tsx#L156-L172)
