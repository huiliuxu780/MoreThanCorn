...
现在让我更新文档，基于新增的验收测试基础设施和 Phase E 测试：
...
## 更新摘要
**变更内容**
- 新增验收测试基础设施：scripts/check-e1-acceptance.mjs、scripts/check-e1-walkthrough.mjs、scripts/check-e2.mjs、scripts/check-e4.mjs 提供关键用户流程的自动化验证
- 扩展测试覆盖范围：test_phase_e.py 包含 Phase E 特定测试，涵盖发布控制面（复制/归档 + 灰度）和观测深化（子 Run span + 首 token）
- 增强浏览器验收测试：通过 Puppeteer 实现端到端 UI 验证，包括角色权限、灰度发布、版本对比等功能
- 完善 API 级验收测试：验证 Agent 创建、版本管理、连接密钥轮换、审计日志等核心功能
...
participant CANARY as "灰度控制"
FE->>API : POST /api/agents/{aid}/releases (canaryPercent=50)
API->>CANARY : 设置灰度比例
CANARY->>DB : 存储 canaryPercent
FE->>API : POST /api/agents/{aid}/run (触发运行)
API->>RUN : create_run(根据 canaryPercent 选择版本)
RUN->>VER : 加载版本快照
RUN->>DB : 写事件(channel/trace/span)
RUN-->>API : workflow_completed/failed
FE->>API : GET /api/agents/{aid}/runs/{runId}/trace
API-->>FE : 包含 agent 子树的完整 trace
```

**图表来源**
- [server/tests/test_phase_e.py:64-113](file://server/tests/test_phase_e.py#L64-L113)
- [server/tests/test_phase_e.py:133-169](file://server/tests/test_phase_e.py#L133-L169)
- [server/tests/test_phase_e.py:172-182](file://server/tests/test_phase_e.py#L172-L182)
