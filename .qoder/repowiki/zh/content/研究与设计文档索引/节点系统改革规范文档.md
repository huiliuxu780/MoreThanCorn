...
## 更新摘要
**变更内容**
- 新增循环容器系统 exec_loop，支持迭代执行、错误处理模式、中断条件和输出聚合
- 新增等待/人审节点 wait-review，实现暂停-恢复原语和人工审核工作流
- 新增数据读取节点 data-read，支持按窗口和抽样策略从数据资产取数
- 增强执行块配置，支持超时、重试和错误分支策略
- 增强 LLM 节点支持 systemPrompt、批处理模式和 JSON 输出格式
- 扩展条件逻辑运算符，支持 in/not_in/exists/is_null 等新操作符
- 完善后端执行器的错误处理和内存变量管理
- 实现向后兼容迁移系统，将 agent/agent-select/agent-exec 迁移至 workflow 系列节点
- 前端新增循环、等待/人审、数据读取节点的配置界面
