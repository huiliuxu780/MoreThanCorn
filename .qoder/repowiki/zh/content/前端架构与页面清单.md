...
**更新** 新增了 Agent 版本对比功能、工作流设计器的节点单测能力、提示提及功能的后端实现，以及预览消息的交互操作增强。
...
### 页面层 (`pages/`)
路由对应的页面组件：
- `wf-designer.tsx`：工作流设计器，支持节点拖拽、连线、配置，**已集成统一评估面板和节点单测功能**
...
**更新** TraceView 组件已实现双模式切换（Span树和步骤视图），改进了错误处理和Token使用可视化，提供了更强大的运行追踪和调试能力。新增了 AgentVersionDiffDialog 组件用于版本对比，工作流设计器集成了节点单测功能。
...
- [src/pages/wf-designer.tsx:1-2585](file://src/pages/wf-designer.tsx#L1-L2585)
...
**更新** 完成了从 Mock 数据到真实后端 API 的全面迁移，移除了约 1475 行的 Mock 服务代码。Connections页面已增强交互体验，工作流设计器已集成统一评估面板和节点单测功能，Run Detail 页面已实现 LangSmith 风格的运行追踪可视化，现已支持双模式查看和增强的错误处理。新增了 Agent 版本对比功能。
...
**更新** 完成了从 Mock 数据到真实后端 API 的全面迁移，所有页面现在都使用真实数据源。Connections页面已增强交互体验，工作流设计器已集成统一评估面板和节点单测功能，Run Detail 页面已实现 LangSmith 风格的运行追踪可视化，现已支持双模式查看和增强的错误处理。新增了 Agent 版本对比功能。
...
1. **Agent 版本管理**：新增了 AgentVersionDiffDialog 组件，支持版本间的 JSON 行级差异对比
2. **工作流设计器增强**：集成了节点单测功能，支持在隔离环境中测试单个节点
3. **提示提及功能**：后端实现了 `#type:name` 语法的自动展开功能
4. **预览消息操作**：增强了预览消息的交互操作，支持复制、点赞、点踩等功能
5. **版本管理集成**：在 Agent 发布对话框中集成了版本对比入口

这些更新进一步完善了平台的版本管理能力、开发调试体验和用户交互功能，为 Agent 和工作流的开发、测试、发布提供了更加完善的工具链支持。
