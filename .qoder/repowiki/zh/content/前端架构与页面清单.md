...
## 更新摘要
**变更内容**
- **Connections页面交互增强**：新增协议分Tab展示功能，支持按HTTP API、LLM、MCP、MySQL、PostgreSQL、OSS等协议分类查看连接
- **测试按钮逐行加载状态**：每个连接的测试按钮独立显示"测试中…"状态，避免全局loading干扰
- **静默刷新机制**：测试完成后仅刷新当前行数据，不触发整页闪烁，提升用户体验
- **Secret字段可见性控制改进**：在创建/编辑对话框中添加显示/隐藏切换按钮，支持密码输入框的可见性控制
- **连接卡片网格布局优化**：采用响应式网格布局，支持1-4列自适应显示
...
**更新** Connections页面已完成交互增强升级，提供更友好的连接管理和测试体验。
...
**更新** Connections页面已完成交互增强，提供更直观的协议分类和更好的测试体验。
...
**更新** Connections页面已完成交互增强，移除了 mock-service.ts，所有页面现在直接使用真实后端 API，提升了系统的生产就绪能力。
...
**更新** 完成了从 Mock 数据到真实后端 API 的全面迁移，移除了约 1475 行的 Mock 服务代码。Connections页面已增强交互体验。
...
**更新** 完成了从 Mock 数据到真实后端 API 的全面迁移，所有页面现在都使用真实数据源。Connections页面已增强交互体验。
