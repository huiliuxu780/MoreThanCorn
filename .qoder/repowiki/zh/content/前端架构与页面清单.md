...
## 更新摘要
**变更内容**
- **新增角色切换功能**：在应用外壳中添加角色切换下拉框，支持 viewer/editor/publisher/admin 四种角色实时切换
- **新增审计日志页面**：创建 `/settings/audit` 路由，仅对具有 `admin.audit` 权限的管理员用户可见
- **增强权限控制**：在导航菜单、UI 操作按钮中集成 RBAC 权限检查，实现细粒度的访问控制
- **条件删除操作**：在 Agent 列表中对删除操作进行权限控制，仅 `agent.edit` 权限用户可执行删除
- **后端审计集成**：所有高危操作（删除、解锁等）均记录审计日志到后端
...
**更新** 新增角色切换功能和审计日志功能，实现了完整的 RBAC 权限控制系统。

## 核心业务流程
- 路由入口与懒加载：应用根在 main.tsx 中创建 BrowserRouter，挂载 AppShell 与 App；app.tsx 通过 React.lazy 对页面进行按需加载，Suspense 提供统一骨架占位。
- 导航与权限：app-sidebar.tsx 维护固定导航分组，结合 rbac.ts 的 can/actionVisibility 控制可见性与动作态；app-shell.tsx 渲染侧边栏、面包屑与内容区，并识别"工作区级"路由隐藏面包屑。**新增角色切换功能**，支持运行时切换用户角色以测试不同权限场景。
...
- [src/services/mock-service.ts:190-354](file://src/services/mock-service.ts#L190-354)
...
- 服务层
  ...
  - rbac.ts：权限策略（当前原型默认放行，真实接入替换 adapter）。**新增四角色权限矩阵和角色切换功能**。
...
- 权限与导航：NAV_GROUPS 固定导航，RBAC 控制可见性与动作态；工作区级路由隐藏面包屑。**新增角色切换功能支持运行时权限测试**。
- 数据可替换：mock-service 与 wf-api/resource-api 提供一致调用形态，可通过环境变量启用真实后端。
- 安全与集成：后端 RBAC 要求 WF_API_TOKEN 时，前端通过 Authorization: Bearer 传递；CORS 仅允许 localhost:5173。**所有高危操作均记录审计日志**。
...
应用级通用组件：
- `app-shell.tsx`：应用外壳，管理侧边栏、面包屑、内容区域。**新增角色切换功能**，支持运行时切换用户角色。
...
- `audit-log.tsx`：**新增**审计日志页面，仅管理员可访问
- `wf-agents-list.tsx`：Agent 列表页面，**新增条件删除操作**
- 其他业务页面：质量结果、任务管理等
...
- `rbac.ts`：权限控制，**新增四角色权限矩阵和角色切换功能**

**更新** 增强了权限控制系统，新增了角色切换功能和审计日志页面。
