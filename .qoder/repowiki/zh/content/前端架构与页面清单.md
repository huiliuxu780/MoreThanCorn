...
## 更新摘要
**变更内容**
- **Run Detail 页面重大重构**：实现了 LangSmith 风格的运行追踪可视化能力，集成了全新的 trace-view 组件
- **四 Tab 界面架构**：Trace（Span 树+瀑布图）、Events（事件流过滤）、Executions（业务结果）、Snapshot（冻结快照）
- **Span 树可视化**：支持真实时间轴瀑布图、节点钻取、输入输出查看、错误传播高亮
- **服务层增强**：新增 runTrace API 端点，提供完整的运行追踪数据组装
...
**更新** Run Detail 页面已完成 LangSmith 风格观测升级，提供强大的运行追踪和调试能力。
...
**更新** Run Detail 页面已实现 LangSmith 风格的运行追踪可视化，提供 Span 树、事件流、业务结果和冻结快照的四 Tab 界面。
...
- **Run 观测升级**：Run Detail 页面提供 LangSmith 风格的运行追踪可视化能力。
...
**章节来源**
- [src/app.tsx:60-111](file://src/app.tsx#L60-L111)
- [src/components/app/app-shell.tsx:166-188](file://src/components/app/app-shell.tsx#L166-188)
- [src/components/app/app-sidebar.tsx:54-138](file://src/components/app/app-sidebar.tsx#L54-L138)
...
- **`run/trace-view.tsx`**：**新增** LangSmith 风格的运行追踪可视化组件，提供 Span 树、瀑布图、事件关联
- `resources/`：资源管理相关组件
...
- **`run-detail.tsx`**：**重大重构** Run 详情页，实现 LangSmith 风格的运行追踪可视化
...
**更新** Run Detail 页面已实现 LangSmith 风格的运行追踪可视化，新增了 `trace-view.tsx` 组件和 `runTrace` API，提供了完整的运行追踪和调试能力。
...
**更新** 完成了从 Mock 数据到真实后端 API 的全面迁移，移除了约 1475 行的 Mock 服务代码。Connections页面已增强交互体验，工作流设计器已集成统一评估面板，Run Detail 页面已实现 LangSmith 风格的运行追踪可视化。
...
**更新** 完成了从 Mock 数据到真实后端 API 的全面迁移，所有页面现在都使用真实数据源。Connections页面已增强交互体验，工作流设计器已集成统一评估面板，Run Detail 页面已实现 LangSmith 风格的运行追踪可视化。
...
### TraceView 组件架构
`trace-view.tsx` 组件实现了完整的 LangSmith 风格可视化：
...
#### 可视化特性
- **双模式切换**：Span 树模式和步骤模式
- **真实时间轴**：基于 startedAt/endedAt 计算精确的时间位置
- **JSON 查看器**：使用 CodeMirror 提供语法高亮的 JSON 查看
- **状态指示**：成功/失败/跳过状态用不同颜色标识
- **Token 可视化**：显示每次调用的 token 消耗
