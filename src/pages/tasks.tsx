...
export default function TasksPage() {
  ...
  return (
    <PageContainer wide className="space-y-3">
      <PageHeader
        title="分析任务"
        description="管理分析任务的数据范围、执行周期和评价 Agent"
        actions={
          canManage ? (
            <Button asChild>
              <Link to="/autonomous-tasks/new">
                <Plus className="size-4" /> 新建分析任务
              </Link>
            </Button>
          ) : null
        }
      ...
      <FilterBar>
        <SearchField value={searchInput} onChange={setSearchInput} placeholder="搜索分析任务..." />
      ...
      {error ? (
        ...
        filters.status || filters.workflow || filters.asset || params.search ? (
          ...
          <EmptyState
            ...
            action={canManage ? <Button asChild><Link to="/autonomous-tasks/new">新建分析任务</Link></Button> : null}
          />
        )
      ) : (
        <>
