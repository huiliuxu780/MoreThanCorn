...
import { Checkbox } from "@/components/beui/motion/checkbox";
import { Input } from "@/components/beui/motion/input";
import { RadioGroup, RadioGroupItem } from "@/components/beui/motion/radio";
...
function getStatusClass(status: ApprovalCardStatus) {
  ...
  if (status === "rejected") return "text-status-danger";
    ...
    <div className="mt-3">
      ...
      {question.allowCustom ? (
        <Input
          value={custom}
          disabled={disabled}
          placeholder={question.customPlaceholder ?? "补充回复…"}
          onChange={(value) =>
            onChange({
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                {onReject ? (
                  <Button
                    ...
                    disabled={busy}
                    onClick={onReject}
                    className="rounded-full text-muted-foreground hover:text-status-danger dark:hover:text-status-danger"
                  >
                    {rejectLabel}
                  </Button>
                ) : null}
