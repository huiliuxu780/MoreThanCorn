...
function getActiveLabel(type: AgentActivityContentType) {
  if (type === "search") return "联网检索中…";
  if (type === "tool") return "工具执行中…";
  if (type === "trace") return "执行中…";
  if (type === "mixed") return "执行中…";
  return "深度思考中…";
}
...
function getSummary(
  ...
  if (type === "trace") {
    ...
    return `${tools} 次工具调用，${messages} 条消息`;
  }
