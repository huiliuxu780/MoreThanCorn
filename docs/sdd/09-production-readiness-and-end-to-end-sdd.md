...
| P1 | **修复轮大部分代码项闭环，整体仍未通过** | 已修（至 `c29bb78`）：SQL 注入/成本聚合/原子复核/重试重汇/队列心跳/告警消费/进程拆分/列表分页/治理水位保留；后端 214 绿。未闭环：前端组件测试/无障碍 + 真实故障注入演练 + §14.1 真实环境项（7 天 Staging/真 Provider Smoke/业务签字）；见 `acceptance/09-p1-acceptance.md` |
| P2 | 进行中（租户已定：单租户隔离） | P2-03 已拍板单租户隔离（不加 tenant 列）；余下待用户决策：10 项 SLO、SSO/OIDC、HA、DR/RPO-RTO、KMS；可先行代码项：P2-02 数据权限/P2-07 PII/P2-08 发布治理 |
