...
- 2026-08-25 实施偏离登记：
  1. migration 016/017 合并为单张 `b026phaseb0001`。
  2. 草稿运行的 CommonConfig 从当前 config 现算（`build_common_config_dict`），与版本快照同构。
  3. 成员冻结版本取成员当前已部署版本（sandbox 优先）；无部署版本的成员运行期回退草稿并发 `member_unfrozen` 事件。
  4. 流式对含 tool_calls 的中间轮次也会发出 content 增量（预览按事件顺序渲染，不影响正确性）。
  5. 删除 Agent 需级联清理 releases/versions（原 FK 缺陷，已修）。

## 12. 状态日志
- 2026-08-25 规格草稿完成，待冻结。
- 2026-08-25 四维反思后修正并重新待冻结。
- 2026-08-25 夜间冻结（用户授权连续施工），开工。
- 2026-08-25 夜间完成：后端（f256bd4）+ 前端（c7f413e）+ 删除级联修复；71/71 pytest 绿；发布→部署→版本运行→回滚冒烟通过。**待用户逐项验收。**
