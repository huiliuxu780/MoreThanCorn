...
| 2026-08-29 | **R3 完成（待用户验收）** | migration `g042r3target0001`（execution_target_type=workflow|agent + agent_id/策略列 + 单条 OR 复合 CHECK 互斥 + TaskRun 冻结列 + quality_result.agent_version_id）；POST /api/tasks 收 executionTarget（旧 payload 兼容）；TaskRun 启动一次解析冻结（Agent 策略 pinned|latest_sandbox|latest_prod，Release 必带绑定，失败关闭）；统一分派 `_interaction_run/_dispatch_interaction_run`（Agent→execute_module_run_sync 内联）；结果事务 exactly-once（Schema 平台二次校验/CallRecord 脱敏映射/QualityResult+Evidence/冻结规则派生评分）；agent-exec 嵌套 Module Agent（子 Run 阶段不造假 NodeRun）。证据：270 pytest（+4）+ 迁移双库 up/down/up + Runtime 9/10 + R0 边界 + verify-fullstack 38/49。偏差：Agent 批次同步内联（异步聚合留容量加固）；绑定快照存 run.runtime_snapshot.runtimeBinding。验收报告 `docs/sdd/acceptance/10-r3-acceptance.md`；未 push/未 `--apply` |
| 2026-08-29 | **R4 完成（待用户验收）** | 后端：Run Detail 增强（runtime/stages/calls/usage/evidence）、`/runtime-providers/metrics/aggregate`（token/P95/cost）、Provider 兼容矩阵、`/api/agents/modules` 目录端点。前端（对位原型 v3）：Catalog 新建对话框+Module 徽标、`module-agent-config` 配置页（身份可编辑/指令资源只读/右侧测试面板）、`module-publish-dialog`（Provider 绑定+灰度）。门禁 test_r4_gates 5 过（Golden Set 评测器/重启恢复/RunDetail/指标/兼容矩阵）。证据：275 pytest + 前端三件套 + verify-fullstack 38/49。偏差：RunDetail/任务向导/Providers 页 UI 下轮补；cost 为估算；真实 Golden Set 需数据授权。验收报告 `docs/sdd/acceptance/10-r4-acceptance.md`；未 push/未 `--apply` |
| 2026-08-29 | **R5 完成（business-analysis 只读 Module，待用户验收）** | 新增只读 Module business-analysis@1.0.0（manifest/Spec/Schema/逻辑工具 read）；Registry 自动发现双 Module；结果事务分流（仅 quality-analysis 写 QualityResult，批次/重汇不变量按目标分流）。证据：test_r5 2 过 + 全量 277 + verify-fullstack 38/49。偏差：business 双 Provider 原生实现以 fake 验证；ticket-automation（写型）待审批/幂等/补偿设计。验收报告 `docs/sdd/acceptance/10-r5-acceptance.md`；未 push/未 `--apply` |
| 2026-08-29 | **口径收紧：一个 Agent 只对应一种 Provider（用户 08-29）** | create_release 增加 `ONE_PROVIDER_PER_AGENT`：已有 active Release 绑定不同 Provider 时拒绝（同 Provider 的灰度/多版本仍允许）；移除"同版本双 Provider 并存"语义。R2 相关测试改为 1:1 断言。全量 277 过。覆盖 SDD §5.4/§10.3 的"双 Provider 并存"表述以此行为准（用户口径优先） |
| 2026-08-29 | **ticket-automation 写型 Module 骨架（待用户验收）** | 新增写型 Module ticket-automation@1.0.0（读写分离：ticket_query read / ticket_update write-reversible / ticket_close write-irreversible；approvalPolicy=high-risk、idempotency=platform-key、compensation=record）。输出 Schema 强制写动作带 idempotency_key+side_effect_verified。写型不写 QualityResult（领域结果=ActionLedger，R6+）。不接真实写操作。证据：test_r6 2 过 + 全量 279。 |
| 2026-08-29 | **R7 完成（Data→Task→Agent→Run 产品闭环，待用户验收）** | 统一 executionTarget 契约（create/list/detail/edit 保真）；Module Catalog 暴露 inputSchema/outputSchema/resultProjection；输入映射必填校验；任务列表/详情/TaskRun→Run 跳转（前端向导默认 Module Agent）；结果路由 quality→QualityResult、business/ticket 不写；重试沿用冻结绑定。证据：test_r7_loop 全链路 + 全量 280 + 前端三件套 + fullstack 38/49。验收报告 `docs/sdd/acceptance/10-r7-acceptance.md`；未 `--apply`/未接真实写/未 push |
| 2026-08-29 | **R7 Data→Task→Agent→Run 闭环（后端，待用户验收）** | R7-1 统一执行目标契约（_execution_target_dto，create/edit/list/detail 返回 executionTarget，agent|workflow 兼容）；R7-3 输入映射校验（_validate_input_mapping：Module 必填输入必须被映射，422 INPUT_MAPPING_INCOMPLETE）；R7-4 任务列表/详情富化（executionTargetType/agentName/moduleKey/最近批次）；闭环测试 test_r7_loop（5 条数据→5 Run→冻结 AgentVersion/Release/Provider→QualityResult exactly-once→失败项重试不重复成功项→汇总一致→新发布不漂移）。不接真实写操作/LLM。证据：test_r7_loop 过 + 全量 280。 |
