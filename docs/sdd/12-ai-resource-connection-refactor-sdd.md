...
| 2026-09-01 | v1.0-P0.2 | 二次验收阻断项修复轮（验收记录 M.7）：① A-03 字段级——`EnvPatch.label/endpoint` 改为真正可缺省（`None`=未提交），`_env_rows_patch` 依据 `model_fields_set` 只覆盖请求实际携带的字段，"只改 label"不再清空 endpoint；② kind 原子性——存在任何凭据（根级或任一环境）时拒绝非原子 kind 变更（422，path=kind），杜绝"字符串 api_key 直接变 basic"的坏状态；前端编辑对话框对已配凭据的连接锁定鉴权方式选择。新增 5 个负向用例与 E2E R10 段（4 断言）。 | 二次独立验收发现字段级 patch 丢配置、kind 变更旁路结构校验 | 后端 341 tests、E2E 45/45 全绿；证据见验收清单 M.8 |
