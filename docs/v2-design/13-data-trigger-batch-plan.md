...
> **状态：`SUPERSEDED`（2026-09-11）**。本文把“攒批层”误判为自动化主链的默认
> 下一步，并高估了当前三执行体闭环成熟度。新的权威方案见
> `docs/product-domain/execution-automation-batch-spec.md`：默认一触发一执行，
> `TaskRun` 只用于分析批跑，入口攒批必须由真实吞吐/成本/SLO 证明后再立项。
